...
# TODO: separate columns by tab (ARMAN, WikiANN corpus) or space (PEYMA corpus, see above) or just use CSV?
...
def extract_verses():
    ...
    for line in shahnameh_text_no_titles.split('\n\n'):
        if not re.search(r'بخش \d+', line) and ":" not in line and line:
            verses.append(line)
    with open('data/shahnameh_cleaned_text.txt', 'w') as clean_txt_file:
        for v in verses:
...
def extract_rhymeless_couplets():
    ...
    with open('data/rhymeless_couplets.txt', 'w') as rhymeless_couplets_file:
...
def get_couplets():
    ...
    pickle_obj(couplets, 'pickles/couplets.pickle')
    return couplets
...
def get_gold_couplets():
    ...
    place_names = csv_as_list('data/places_gold_list.csv')
    characters_names = csv_as_list('data/characters_gold_list.csv')
    ...
    with open('data/gold_couplets.txt', 'w') as gold_couplets_file:
        gold_couplets_file.write('\n'.join(gold_couplets))

    pickle_obj(gold_couplets, 'pickles/gold_couplets.pickle')
    return gold_couplets


# TODO: shorten + clean up
# => 370 new names found
def replace_spaces_in_names():
    g_couplets = unpickle_obj('pickles/gold_couplets.pickle')
    p_names = csv_as_list('data/places_gold_list.csv')
    c_names = csv_as_list('data/characters_gold_list.csv')
    space_names = {}
    modified_g_couplets = []
    modified_p_names = []
    modified_c_names = []
    ...
    pickle_obj(modified_g_couplets, 'pickles/modified_g_couplets.pickle')
    pickle_obj(modified_p_names, 'pickles/modified_p_names.pickle')
    pickle_obj(modified_c_names, 'pickles/modified_c_names.pickle')
    return modified_g_couplets, modified_p_names, modified_c_names


# TODO: shorten + clean up
def annotate_gold_couplets():
    gold_couplets = unpickle_obj('pickles/modified_g_couplets.pickle')
    loc_names = unpickle_obj('pickles/modified_p_names.pickle')
    per_names = unpickle_obj('pickles/modified_c_names.pickle')
    n = 0
    ...
    pickle_obj(tagged_couplets_list, 'pickles/tagged_couplets_list_final.pickle')
    return tagged_couplets_list


def save_annotations(tagged_couplets_lst):
    with open('results/auto_annot_training_data_final.txt', 'w') as annot_file:
...
if __name__ == '__main__':
    ...
    save_annotations(unpickle_obj('pickles/tagged_couplets_list_final.pickle'))
